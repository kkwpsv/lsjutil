...
namespace Lsj.Util.Win32.Structs
{
    /// <summary>
    /// <para>
    /// The <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE"/> structure defines an access control entry (ACE)
    /// that controls allowed access to an object, property set, or property.
    ...
    /// each <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE"/> structure contained in the DACL of a <see cref="SECURITY_DESCRIPTOR"/> structure
    ...
    /// in which a pointer to the <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE"/> structure found is passed in the pAce parameter.
    /// </para>
    /// <para>
    /// From: <see href="https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-access_allowed_callback_object_ace"/>
    /// </para>
    /// </summary>
    /// <remarks>
    /// If neither the <see cref="ObjectType"/> nor <see cref="InheritedObjectType"/> GUID is specified,
    /// the <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE"/> structure has the same semantics as those used by the <see cref="ACCESS_ALLOWED_CALLBACK_ACE"/> structure.
    /// In that case, use the <see cref="ACCESS_ALLOWED_CALLBACK_ACE"/> structure because it is smaller and more efficient.
    /// An ACL that contains an <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE"/> must specify the <see cref="ACL_REVISION_DS"/> revision number in its ACL header.
    ...
    /// When an <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE"/> structure is created,
    ...
    public struct ACCESS_ALLOWED_CALLBACK_OBJECT_ACE
    {
        ...
        /// The <see cref="ACE_HEADER.AceType"/> member of the <see cref="ACE_HEADER"/> structure should be set to <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE_TYPE"/>,
        /// and the <see cref="ACE_HEADER.AceSize"/> member should be set to the total number of bytes allocated for the <see cref="ACCESS_ALLOWED_CALLBACK_OBJECT_ACE"/> structure.
        /// </summary>
